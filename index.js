...
function getArtistByIndex(array, n) {
  return `the artist at index ${n} is ${array[n].name}`;
}  
...
function get20s(array){
  ...
  for (let i = 0; i < array.length; i++) {
    let arrTemp = array[i].years.split(" ");
    if (parseInt(arrTemp[0]) >= 1900 && parseInt(arrTemp[2]) <= 2000) {
      store.push(array[i].name);
    }
  }
  return store;
}
...
function addArtist(array) {
    array.push({
      ...
      "bio": "Wow what an amazing dude. Intensly specefic bio here: engage",
    });
    return array;
  }
